...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    body: "Для входа нужны одновременно: |score| от 0.30 до 0.62 без перегрева, confidence ≥ 60%, согласие ≥ 80%, минимум 5 из 6 факторов по направлению, нормальный режим волатильности, подтверждение старшего тренда и окно первых 60 секунд. Сделок становится в разы меньше, но уходят импульсные ложные входы, которые давали ~57% точности.",
  },
  {
    index: "03",
  ...
  },
  {
    ...
    body: "Confidence не является вероятностью и не задаёт цену контракта. Вход разрешён только в early-фазе, при достаточном согласии факторов, нормальной волатильности, подтверждённом старшем тренде и буфере от открытия раунда; вероятность и max price рассчитываются отдельно.",
    items: [
      { label: "Тренд EMA 15/40 на 1м должен совпадать с вызовом", value: "обязательно" },
      { label: "Цена уже отошла от открытия раунда на ≥ 0.35 ATR", value: "обязательно" },
