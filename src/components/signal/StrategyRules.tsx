...
const RULES: Rule[] = [
  {
    index: "01",
    title: "Вызов только в начале раунда",
    body: "Движок считает перевес сразу после открытия раунда и публикует вызов, пока не прошло 150 секунд. Лучшее окно — первые 60 секунд: контракт ещё не переоценён. Дальше вход запрещён, даже если направление кажется очевидным.",
  },
  {
    index: "02",
    title: "Два порога на вход",
    body: "Вызов публикуется, только если перевес |score| ≥ 0.22 и уверенность ≥ 60%. Уверенность — это сила сигнала и согласие факторов, а не вероятность. Цену входа считает отдельная оценка вероятности (около 0.50–0.60): максимальная цена входа = оценка − 6 п.п. Если контракт на Binance стоит дороже — сделки нет.",
  },
  {
    index: "03",
  ...
  },
  {
    index: "04",
    title: "Фильтры риска",
    body: "Факторы дают направление, фильтры срезают уверенность — а значит, шанс, что вызов вообще пройдёт порог публикации.",
    items: [
  ...
  {
    index: "05",
    title: "Затухание по времени",
    body: "Чем позже в раунде снят вызов, тем меньше от него проку: рынок уже оценил движение. Уверенность затухает вместе с окном — 60–150 секунд это ×0.82, поздняя фаза ×0.58 и полный запрет входа.",
  },
  {
    index: "06",
