...
const TIMELINE = [
  {
    time: "12:04:58",
    title: "Подготовка",
    body: "Движок держит в памяти последние 200 минутных свечей — к границе раунда он уже готов считать.",
  },
  {
    time: "12:05:00",
    title: "Раунд открылся",
    body: "Зафиксирована цена открытия раунда — именно с ней контракт сравнит результат. Вход пока запрещён: первой минуты ещё нет.",
  },
  {
    time: "12:06:00",
    title: "Первая минута закрыта",
    body: "Первый настоящий факт о раунде: цена ушла от открытия на 0.7 ATR вверх. Шесть факторов голосуют, перевес +0.34.",
  },
  {
    time: "12:06:04",
    title: "Вызов зафиксирован",
    body: "UP, уверенность 71%, максимальная цена входа 0.52. Запись уходит в журнал и больше не меняется.",
  },
  {
    time: "12:06:05",
    title: "Вход",
    body: "Контракт на Binance стоит 0.49 — ниже нашего лимита, перевес в нашу пользу. Позиция взята на 66-й секунде раунда.",
  },
  {
    time: "12:08:00",
    title: "Окно закрыто",
    body: "После 120-й секунды новые входы запрещены: контракт уже отражает движение, перевес уходит рынку.",
  },
  {
    time: "12:10:00",
    title: "Расчёт",
    body: "Раунд закрылся выше цены открытия — вызов ушёл в плюс и попал в статистику точности.",
  },
];

const DISCIPLINE = [
  {
    title: "Максимальная цена входа вместо «надо успеть»",
    body: "Уверенность минус 6 процентных пунктов — красная черта. Контракт дороже — сделки нет, даже если направление совпало.",
    icon: ShieldCheck,
  },
  {
    title: "Затухание уверенности",
    body: "Окно подтверждения — 60–120 секунд. Дальше коэффициент 0.82, с 180-й секунды — 0.58 и полный запрет входа: поздний вызов ничего не стоит.",
    icon: Clock,
  },
  {
    title: "Жёсткий стоп по ATR",
    body: "Если отрыв от открытия раунда ушёл против вызова на 0.6 ATR, убыток фиксируется сразу, а не дожидается закрытия.",
    icon: ArrowDown,
  },
  {
    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="mx-auto flex w-full max-w-6xl items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] leading-tight text-muted-foreground">
              Сигнал Up/Down · 5 минут · BTC и ETH
            </p>
          </div>
        ...
        <div className="relative mx-auto grid w-full max-w-6xl items-center gap-12 px-4 pt-16 pb-20 sm:px-6 lg:grid-cols-[1.08fr_1fr] lg:pt-24 lg:pb-28">
          ...
          >
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              Binance Prediction Market · 5-минутные раунды
            </span>
            <h1 className="mt-5 text-[2.5rem] leading-[1.05] font-semibold tracking-tight text-balance sm:text-5xl lg:text-[3.4rem]">
              Вызов Up или Down — по факту первой минуты, пока контракт ещё дешёвый
            </h1>
            <p className="mt-5 max-w-xl text-sm leading-relaxed text-muted-foreground sm:text-base">
              Первые секунды раунда — это шум тиков: на реальных данных Binance
              вызовы, сделанные внутри первой минуты, угадывали около 64%. Early
              Push дожидается закрытия первой минуты, считает перевес по шести
              факторам и публикует один вызов с настоящим отрывом от цены
              открытия — на прогоне за 30 дней это около 73%.
            </p>
      ...
      <section className="border-t border-border/70 bg-card/50">
        <div className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
          <SectionHeading
            eyebrow="Почему момент входа решает"
            title="Внутри первой минуты вызов ставить не на что"
            body="Поздний вход бесполезен — контракт уже всё знает. Но и самый ранний вход не работает: пока первая минута не закрылась, у движка нет ни одного факта о раунде, только тики. Перевес появляется ровно между этими двумя точками."
          />
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {[
              {
                title: "Первая минута — это шум",
                body: "Внутри первых 60 секунд цена успевает сходить в обе стороны, а контракт всё ещё стоит около 0.50. Вход по такому движению — ставка на монетку: на историческом прогоне такие вызовы дали около 64%.",
                icon: Clock,
              },
              {
                title: "Закрытая минута — это факт",
                body: "Когда первая минута раунда закрылась, видно реальный отрыв от цены открытия. Требуя минимум 0.5 ATR отрыва, движок берёт только раунды с подтверждённым движением — около 73% попаданий.",
                icon: Activity,
              },
              {
                title: "Пропуск раунда — часть системы",
                body: "Нет перевеса — нет вызова. Движок умеет сказать «скип», и статистика точности считается только по реально опубликованным вызовам.",
                icon: ShieldCheck,
              },
            ].map((item, index) => (
              <motion.article
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 px-4 py-8 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground">
              Early Push — сигнальная стратегия для 5-минутных рынков BTC и ETH
            </span>
          </div>
